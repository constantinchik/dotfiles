...
For each item, use the `question` tool to present it individually:
...
For each confirmed item, invoke the `note-taker` subagent with clear instructions: what to write, the note type, proposed title, and the content. The agent knows all vault rules - do not repeat them here.
