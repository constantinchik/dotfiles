...
Synchronize the OpenCode `note-taker` agent definition with the actual Obsidian vault structure, templates, and conventions.
...
- Use Glob to find `~/.config/opencode/agents/note-taker.md`
- Read its full contents and identify each section by `##` headers

### 2. Scan the vault folder structure

- Call `obsidian_list_files_in_dir` for each top-level PARA folder:
  - `1. Projects/`
  - `2. Areas/`
  - `3. Resources/`
  - `4. Archive/`
...
- Use `obsidian_batch_get_file_contents` to read all template files
- Extract from them:
...
Use the `question` tool to show a clear summary of all detected changes:
...
Use targeted edits to update **only the affected sections** of the agent file. Key rules:
