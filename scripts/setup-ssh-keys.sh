...
show_client_setup_instructions() {
    ...
    echo "   Copy the private key to your MacBook:"
    echo "     scp ${wsl_user}@${wsl_ip}:${KEY_MACBOOK} ~/.ssh/id_wsl"
    echo ""
    echo "   Add to your MacBook's ~/.ssh/config:"
    echo "     Host wsl"
    echo "         HostName ${wsl_ip}"
    echo "         User ${wsl_user}"
    echo "         IdentityFile ~/.ssh/id_wsl"
    echo ""
    echo "   Then connect with:"
    echo "     ssh wsl"
    echo ""
    echo -e "${GREEN}2. n8n Server Setup:${NC}"
    echo "   Copy the private key to your n8n server:"
    echo "     scp ${wsl_user}@${wsl_ip}:${KEY_N8N} /path/to/n8n/keys/"
    echo ""
    echo "   Configure n8n SSH credentials with:"
    echo "     Host: ${wsl_ip}"
    echo "     User: ${wsl_user}"
    echo "     Private Key: contents of ${KEY_N8N}"
    echo ""
